...
namespace XtermSharp {
	public static class RuneHelper {
		...
		// The table is a flat run of {lo, hi} pairs: entry i is table [i * 2] and
		// table [i * 2 + 1]. It used to be uint[,], but multidimensional arrays need
		// runtime support the kernel tier does not ship.
		static int bisearch (uint rune, uint [] table, int max)
		{
			int min = 0;
			int mid;

			if (rune < table [0]|| rune> table [max * 2 + 1])
				return 0;
			while (max >= min) {
				mid = (min + max) / 2;
				if (rune > table [mid * 2 + 1])
					min = mid + 1;
				else if (rune < table [mid * 2])
					max = mid - 1;
				else
					return 1;
			}
		...
		public static int ConsoleWidth (this uint rune)
		{
			...
			if (bisearch (rune, combining, combining.Length / 2 - 1) != 0)
				return 0;
