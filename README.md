...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/vectors-5845%2F5945-f2640a?style=flat-square&labelColor=30363d" alt="vectors replayed: 5845 of 5945">
...
Vectors before code. The harness for monero's `tests/crypto/tests.txt` was the first thing committed, so every crypto file since is born under the whole corpus: 5945 lines, 20 operations, 5845 replayed line by line — including monero's deterministic test generator, reproduced byte for byte, draw pattern included. Corpus tests assert the vector files themselves are intact, because a truncated vector file is the one failure that makes every later test pass for free. Inventory in [docs/test-corpus.md](docs/test-corpus.md).
